...
- **Set on wrong axis** — the value the yaw loop reads is `GantryYawRef` on the **yaw axis** (`B`); writes on the master axis or on a non-gantry axis are accepted but never read.
- **Motor off** — accepted; the value sits until both motors are on and gantry is engaged.
- **Out of range** — writes outside the -20000 to 20000 range are rejected; the value is not clamped.
