...
- **At gantry-off transition** — the firmware unwinds `GantryYawRef` into the per-motor position references during recombination (`PosRefShapedFilt[A] += YawRef/2`, `PosRefShapedFilt[B] -= YawRef/2`), so a non-zero yaw at the moment of disengagement leaves the two motors at a corresponding mechanical offset.
