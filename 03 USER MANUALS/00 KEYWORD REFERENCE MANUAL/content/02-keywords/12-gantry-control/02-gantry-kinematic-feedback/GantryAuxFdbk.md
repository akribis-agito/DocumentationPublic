...
- **Motor off** — the gantry calculation halts for the pair; if A and B disagree on motor state the controller forces the still-on member off and records [ConFlt](../../07-status-and-faults/ConFlt.md) code `1061` (other gantry member axis got motor off).
