...
It is readable and writable at any time, including while in motion and with the motor on. Values are set with a normal write. The host-side indirect-write mechanism (`IndirectArray` / `IndirectIndex` / `IndirectValue` with `IndirectDo`) cannot target this array; it writes only the 32-bit integer [GenData](GenData.md). Within a user program, an element can still be addressed with a computed (run-time) index — see [Addressing](GenData.md#addressing). The array is 1-indexed: the first usable element is `GenDataLL[1]` (index 0 is reserved and inaccessible), and there are 100 usable elements. Although stored as 64-bit integers, the accepted value range is limited to -2251799813685248 to 2251799813685247 (a 52-bit signed range) so that every value remains exactly representable as a double-precision number for host data recording.
