...
| Value | Digital incremental encoder type | Firmware constant |
|-------|----------------------------------|-------------------|
| 0     | A quad B encoder (AqB)           | `ENCODER_SUB_TYPE_AB`      |
| 1     | Pulse-direction encoder          | `ENCODER_SUB_TYPE_PD`      |
| 2     | C0/C1 bits                       | `ENCODER_SUB_TYPE_C0C1`    |
| 3     | Up/down pulses                   | `ENCODER_SUB_TYPE_UP_DOWN` |

When `EncSubType` is written, the firmware programs the encoder-decoding hardware with the selected scheme. The subtype is packed together with the input filter ([EncFilt](EncFilt-AuxEncFilt.md)) and direction ([EncDir](EncDir-AuxEncDir.md)) into a single configuration word: the subtype occupies the low byte, and the filter and direction occupy the next bytes (`SpecialFuncs.c:571`). On central-i this word is sent as a remote-encoder configuration message; on the standalone controller it is written to the encoder-setting register. Because the subtype, filter and direction share one word, changing any of them rewrites the whole configuration.
...
`AuxEncSubType` selects the decoding scheme for the auxiliary incremental encoder, using the same value table, and is packed into the auxiliary encoder configuration word alongside `AuxEncFilt` and `AuxEncDir` (`SpecialFuncs.c:356`, `:383`).
