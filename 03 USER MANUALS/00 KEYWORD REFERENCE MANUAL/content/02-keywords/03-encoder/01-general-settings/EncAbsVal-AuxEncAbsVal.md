...
- **Power-up seed.** For a fixed period after power-up the firmware seeds the accumulated position from `EncAbsVal + EncAbsOff`. On the standalone controller this window is governed by a single shared counter and lasts about one second (correspondingly shorter on a multi-axis controller); on Central-i each axis has its own window (re-armed whenever the port is reconfigured) spanning roughly the first 150 cycles after the port is configured. In all cases the seed has completed well before the axis is enabled.
