...
Each control cycle, after the feedback has been decoded and error-mapped, the firmware checks the position against the modulo boundaries (`AG300_CTL01ControlInterrupt.c`, around line 3640):

- **High side** — when `Pos ≥ ModRev` (or, for a stepper/`MotorType=6`, when the final reference reaches `ModRev`), `ModRev` is **subtracted** from `Pos`, `EncoderPos`, `PosPrev`, and from every reference variable (`PosRef`, `FinalPosRef`, `AbsTrgt`, the shaped/filtered references, `PDPos`, `MasterPos`). The references scaled in 50.14 fixed point are offset by `ModRev << SampleFreqPower`; `PDPos` (32.32) is offset by `ModRev << 32`.
- **Zero side** — when `Pos < 0`, the same set of variables has `ModRev` **added** back.

Because the whole reference frame moves together, `PosErr = PosRef − Pos` is unchanged by the wrap. The functions that perform this are `ApplyModRevHighSide()` and `ApplyModRevZeroSide()` (`AG300_CTL01ControlInterrupt.c:13138` and `:13193`).
...
- **Jerk buffer.** The wrap is held off until the jerk buffer is clear of pre-wrap values (`glNumberOfWrongValuesInJerkBufferBecauseOfModulus`). For truly endless motion the time for one full `ModRev` at max speed must exceed the jerk time ($2^{Jerk}$ samples) — set `ModRev` large enough.
...
- **Software limits.** `ModRev` must lie within the software position limits — it is rejected (`MODREV_IS_OUT_OF_SW_LIM_RANGE`) if `ModRev < RevPLim` or `ModRev > FwdPLim` (`AG300_CTL01Funcs.c:1074`).
...
In **v5** the feedback pipeline moved to 64-bit, so the wrap arithmetic in `ApplyModRevHighSide()` / `ApplyModRevZeroSide()` operates on a 64-bit `Pos` (`gllPos`) and the references. The divisor range is unchanged. **v5 is central-i only.**
