...
`EncAbsRData` 包含由 [EncAbsSendCmd](EncAbsSendCmd.md) 触发的绝对式编码器寄存器读取事务所返回的字节。读取成功后，由 [EncAbsAddr](EncAbsAddr.md) 寻址的编码器寄存器的值可在此处获取。该值为 8 位（0 到 255）。它是只读、轴相关参数，不保存至闪存。适用于 v4（独立式与 central-i）和 v5（central-i）；两个版本的 CAN 代码不同（参见[版本间的变化](#版本间的变化)）。
