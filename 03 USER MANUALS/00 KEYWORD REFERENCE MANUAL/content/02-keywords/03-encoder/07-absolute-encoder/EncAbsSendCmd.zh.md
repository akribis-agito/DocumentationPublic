...
任一分支完成后，固件将编码器接口命令返回其空闲（正常位置读出）状态，然后向上位机回复 OK。寄存器和数据均为 8 位（0–255）。

> [!caution]
> 在 **v4** central-i 主控上，该命令会被接受，但事务无法到达编码器：central-i 远程单元只通过其编码器配置字接收 Tamagawa 命令、地址和写入数据，而 `EncAbsSendCmd` 并未使用该配置字，因此 [EncAbsRData](EncAbsRData.md) 中不是编码器数据。请勿在 v4 central-i 主控上使用 `EncAbsSendCmd`。**v5** 在 central-i 主控上会拒绝该命令（见下文）。
...
|---|---|---|
| CAN 代码 | 719 | 902 |
| 在 central-i 主控上 | 接受，但事务无法到达编码器 | 拒绝，返回错误 242（“This function is not supported in this controller type”） |
