...
After either branch the firmware returns the encoder-interface command to its idle (normal position-readout) state, then replies OK to the host. Registers and data are 8-bit (0–255).

> [!caution]
> On a **v4** central-i master the command is accepted, but the transaction does not reach the encoder: the central-i remote unit takes the Tamagawa command, address and write data only through its encoder configuration word, which `EncAbsSendCmd` does not use, so [EncAbsRData](EncAbsRData.md) does not hold encoder data. Do not use `EncAbsSendCmd` on a v4 central-i master. **v5** refuses it on a central-i master (see below).
...
|---|---|---|
| CAN code | 719 | 902 |
| On a central-i master | Accepted, but the transaction does not reach the encoder | Refused with error 242 ("This function is not supported in this controller type") |
