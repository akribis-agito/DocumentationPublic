...
`HWTimer` 是一个快速、只读的自由运行计数器，用于以亚微秒分辨率对短时间间隔计时。它每微秒大约变化 433 次（大约每 1/433 µs 计数一次）。该计数器**递减**计数，因此在两个时刻读取它，并用较早的读数减去较晚的读数，即可得到两个事件之间的经过时间。计数器在约 9.9 秒后环绕，因此仅适用于短间隔；对于更长或粗粒度的计时请使用 [Time](Time.md)，对于以整个控制周期计数请使用 [CounterUp](CounterUp.md) / [CounterDown](CounterDown.md)。
