...
3. **移除。** 只有在连接仍然有效、且整个数据包已交给控制器的 TCP 协议栈发送之后，事件才会从队列中移除。如果发送被拒绝（例如没有上位机连接），或者上位机在确认该数据包之前关闭或重置了连接，事件保留在队列中，控制器在之后的轮次重试，发送给下一个连接的客户端。
...
当推送事件数据包和 `printf` 数据包同时就绪时，先发送事件数据包，`printf` 等待之后的轮次。在控制器尝试发送推送事件的任何轮次中，无论发送是否成功，`printf` 都不发送，因此一个轮次在推送套接字上最多只进行一次发送。`PushEvFlushMs` 和 `PushEvFillPct` 仅作用于事件；`printf` 输出保持其自身固定的发送时序。

> **连接推送端口的上位机必须持续读取。** 推送事件与 `printf` 使用相同的阻塞式发送。如果上位机已连接端口 50010 但停止读取，每次发送尝试都可能使控制器的后台循环停顿最长约 6 秒。控制器在每个后台轮次最多进行一次此类尝试，并在之后的轮次中持续重试，直到上位机恢复读取或断开连接。在后台循环停顿期间，来自以太网命令端口和串口的命令需等待，包括关闭推送事件的命令；Modbus TCP 请求仍会得到处理。后台看门狗在 10 秒内没有完成一个轮次时触发，预计不会被触发，但该上限是根据固件设计推导得出的，尚未在硬件上实测。请勿让客户端连接端口 50010 却不读取。
...
如果发送在中途被打断（例如连接断开），数据包可能不完整。客户端如果发现数据流中有一段不是完整的数据包，应重新同步：查找下一个包封（`1.<SOURCE>,<version>.`；`SOURCE = 1` 的 `printf` 数据包共用同一数据流），并用 `n` 和 `7 + 50 × n` 的长度加以确认。
