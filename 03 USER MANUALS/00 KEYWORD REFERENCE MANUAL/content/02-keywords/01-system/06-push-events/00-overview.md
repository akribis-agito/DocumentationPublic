...
3. **Removal.** Events are removed from the queue only once the whole packet has been handed to the controller's TCP stack for sending while the connection is still up. If a send is refused, for example because no host is connected, or if the host closes or resets the connection before acknowledging the packet, the events stay queued and the controller tries again on a later pass, for the next client that connects.
