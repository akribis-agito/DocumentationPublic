...
$$
ForceRef = a \times command + (1 - a) \times ForceRef_{prev}
$$
...
$$
a = 1 - e^{-2\pi \, T_s \, (ForceRefFilt / 100)}
$$

where `Ts` is the controller sample time. A higher `ForceRefFilt` gives a higher cut-off and a faster reference response; a lower value smooths the command more. When the filter is disabled (`ForceRefFOn = 0`) the coefficient is forced to `a = 1`, so `ForceRef` equals the raw command unfiltered.
