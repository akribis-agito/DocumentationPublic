...
`LmFFWLevel` scales only this L·di/dt inductive contribution to the quadrature- and direct-axis feedforward outputs [VqFFW](VqFFW.md) and [VdFFW](VdFFW.md). Those outputs also carry a speed-dependent d-q cross-coupling term, but that term is not set by `LmFFWLevel` (see [How it works](#how-it-works)). The inductive term is only applied when voltage feedforward is enabled by [VoltageFFWOn](VoltageFFWOn.md).
