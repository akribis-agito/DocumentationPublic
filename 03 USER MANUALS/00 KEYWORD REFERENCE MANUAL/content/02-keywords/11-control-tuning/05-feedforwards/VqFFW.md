...
| $X_{\text{ffw}}\,i_{d,\text{ref}}\,\omega$ | Cross-coupling: the speed-dependent coupling from the d-axis current into the q axis (derived from the inductance term) | [Lm](../../../02-keywords/09-current-and-voltage/04-motor-measurement/Lm.md) / [LmFFWLevel](LmFFWLevel.md) |
...
`VqFFW` is reset to 0 when the current loop is reset. All three level scalings ([RmFFWLevel](RmFFWLevel.md), [LmFFWLevel](LmFFWLevel.md), [BEMFFFWLevel](BEMFFFWLevel.md)) default to 0, so with default settings every term is zero and `VqFFW` reads 0.
