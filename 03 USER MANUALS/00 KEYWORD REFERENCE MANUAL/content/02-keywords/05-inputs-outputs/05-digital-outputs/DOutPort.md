...
`DOutPort` is the single word that the control interrupt drives onto the physical output pins every cycle. At the end of each control interrupt the firmware computes the *final* output word and writes it straight to the FPGA discrete-output register:

$$
DOutPortFinal = DOutPort \oplus DOutLog
$$

So `DOutPort` is the raw on/off intent and [DOutLog](DOutLog.md) is the per-bit polarity applied on top of it. On controllers whose output stage has selectable sink/source pins (open-collector outputs), the firmware then splits `DOutPortFinal` by [DOutType](DOutType.md): the sink-driver register gets the bits where `DOutType = 0` and the source-driver register gets the bits where `DOutType = 1`. Differential outputs occupy the high bits of the same word.

`DOutPort` is not just written by you. When an output is assigned a software function through [DOutMode](DOutMode.md), the control interrupt *itself* sets or clears the corresponding `DOutPort` bit each cycle to mirror the chosen status — which is why a bit you write by hand is overwritten as soon as `DOutMode` for that output is non-zero. Manual control therefore requires `DOutMode[x] = 0` so that nothing else owns the bit.
...
1. Writing the whole word (`ADOutPort=value`) replaces every bit at once. To change one bit while leaving the others, the read–modify–write idiom `DOutPort = DOutPort | Bitword` works but is **not safe against the control interrupt**, which can write `DOutPort` (for `DOutMode` functions) between your read and your write. Prefer the [DOutPortSBit/CBit/TBit](DOutPortSBit-DOutPortCBit-DOutPortTBit.md) operations, which perform the bit change with interrupts disabled.
