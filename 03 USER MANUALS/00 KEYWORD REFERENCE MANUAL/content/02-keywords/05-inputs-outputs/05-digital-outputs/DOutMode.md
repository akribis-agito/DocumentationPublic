...
When you write `DOutMode`, the firmware does not act on it directly each cycle. Instead it rebuilds a compact **functionality table**: it scans `DOutMode` for every output whose lower-16-bit function is non-zero, and for each one records a target `DOutPort` bit (a set-mask and its complement clear-mask), the function code, and the source axis. The new table is copied into the live table with interrupts disabled so a half-built table is never used. There is a hard limit of **18 active output functions** at once; beyond that the extra entries are dropped and a warning is logged.

The control interrupt then walks this table and applies each function, spread across three consecutive control cycles (about six functions per cycle) to bound the per-cycle cost. For each entry it evaluates the selected status and either **sets** the output's [DOutPort](DOutPort.md) bit (status true) or **clears** it (status false). Because the function drives `DOutPort`, [DOutLog](DOutLog.md) polarity and [DOutType](DOutType.md) sink/source routing still apply on top, exactly as for a manual output. This also means a `DOutMode`-driven output overwrites any value you write to that `DOutPort` bit by hand.
