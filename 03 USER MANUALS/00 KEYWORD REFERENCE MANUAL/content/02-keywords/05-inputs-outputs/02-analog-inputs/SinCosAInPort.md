...
> **Removed in earlier firmware.** `SinCosAInPort` is no longer a supported keyword
> and cannot be used. This page is retained for reference to older firmware and documentation.
...
It was removed when support for the dedicated `SinCosSignals[1-6]` keyword was added;
sin/cos feedback is no longer routed through the general analog-input ports.
It is no longer a supported keyword on current firmware.
