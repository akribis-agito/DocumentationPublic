...
- **Out-of-range function** — values with lower 16 bits > 10 are rejected: the entry is zeroed and fault 2013 (the functionality assigned to AInMode exceeds the allowed range) is logged.
...
- **Wrong-mode use** — assigning function 1 (velocity command) does not by itself switch the axis into velocity-control [OperationMode](../../08-axis-operation/01-general-keywords/OperationMode.md); the routed value is only used while the consuming mode is active. The route is harmless in other modes.
