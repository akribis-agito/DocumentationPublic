...
`RefOffsetSamp` is a **countdown of how many servo cycles still inject the offset**. While `RefOffsetSamp > 0` and the axis is in motion (and not in a stop/abort process), each control cycle the firmware (`AG300_CTL01ControlInterrupt.c:3775`):

1. decrements `RefOffsetSamp` by one, and
2. adds [RefOffsetStep](RefOffsetStep.md) to the high-precision reference accumulator `gllPosRef` (then re-derives [PosRef](../01-kinematics-status/PosRef.md) from it).
...
The correction is applied **only during active motion**. If the motion ends (or a stop is requested) before the count expires, the firmware sets `RefOffsetSamp = 0` (`:3788`) so a leftover offset is not carried into the next motion.
