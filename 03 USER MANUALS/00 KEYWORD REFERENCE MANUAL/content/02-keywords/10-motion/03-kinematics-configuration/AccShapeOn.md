...
- **Other motion modes:** consulted only by the PTP / repetitive-PTP profiler and by the indirect position-following and indirect gear-following profilers. Jog, joystick-velocity and all of the direct modes do **not** consult the shaping table and ignore `AccShapeOn`. Where it is applied, the shaping factor scales the acceleration/deceleration before the profiler branches on [JerkMode](../02-motion-configuration/JerkMode.md), so it also affects the third-order jerk profiler (`JerkMode = 1`) for those moves.
