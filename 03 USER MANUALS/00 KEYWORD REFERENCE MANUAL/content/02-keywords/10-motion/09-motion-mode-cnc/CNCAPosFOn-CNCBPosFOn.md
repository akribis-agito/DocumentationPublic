...
When a CNC path begins, the controller validates the combination of `CNCAPosFOn` and [CNCAPosFDef/CNCBPosFDef](CNCAPosFDef-CNCBPosFDef.md): if the filter is enabled, the definition must describe a valid filter or the move is rejected. Set up the definition first, then enable the filter. Writing this keyword directly while a path is running updates the stored value but does not retune the live filter on its own; the new setting is applied at the next path start. The filter only does any work while a CNC path is running. To switch the filter on or off mid-path, a path program queues a dedicated parameter-change segment that updates the same enable flag and definition; when that segment is reached the controller recomputes the working coefficients in the background, without interrupting the queue.
