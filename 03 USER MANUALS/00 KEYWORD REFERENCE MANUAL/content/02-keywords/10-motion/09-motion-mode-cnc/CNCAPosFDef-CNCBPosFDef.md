...
The array uses the controller's standard customisable position-filter definition: the first element selects the filter **type**, and the remaining elements supply up to five **parameters** for that type. A type of `0` (the default) means no filter, so the path reference passes through unchanged. When a filter type is selected and [CNCAPosFOn/CNCBPosFOn](CNCAPosFOn-CNCBPosFOn.md) = 1, the controller derives the working coefficients from these parameters and applies a second-order (biquad-style) smoothing filter to the resultant path reference. The same definition convention is used elsewhere on the controller for customisable position filters.
