...
- It is therefore a cumulative tally over the life of the current queue, not a per-segment value and not a threshold. Writes are limited to the range 0–1, so the only useful write is `=0` to clear the tally; any value above 1 is rejected as out of range. Normal use is to read it after streaming a path and check whether it is still 0.
