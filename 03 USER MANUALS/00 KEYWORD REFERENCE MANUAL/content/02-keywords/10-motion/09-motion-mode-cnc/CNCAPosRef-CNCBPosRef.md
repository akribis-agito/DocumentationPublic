...
The accumulation is carried internally at higher precision than user units so that fractional path motion builds up without drift; `CNCAPosRef` is reported by scaling that value back to user units. The extra precision is also used when splitting the path position across axes, so the axes stay sub-count accurate on the path. An optional path-position smoothing filter (see [CNCAPosFOn/CNCBPosFOn](CNCAPosFOn-CNCBPosFOn.md)) can be applied to the reference before it is distributed to the axes. When that filter is enabled, the value reported by `CNCAPosRef` is the filtered path reference — the same smoothed value that is fed to the member axes — not the raw pre-filter profile; with the filter off the two are identical. Likewise, [CNCAdPosRef/CNCBdPosRef](CNCAdPosRef-CNCBdPosRef.md) is the per-cycle change of this reported (filtered) reference.
