...
When the pair is set, the controller reduces `MasterFact` and `MasterFactDen` by their greatest common divisor. The denominator is always kept positive — the ratio's sign lives in `MasterFact`.
