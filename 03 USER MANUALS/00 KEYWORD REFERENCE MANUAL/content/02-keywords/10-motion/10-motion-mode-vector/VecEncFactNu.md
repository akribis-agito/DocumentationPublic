...
Set the pair so that `VecEncFactNu / VecEncFactDn` equals the resolution ratio needed for the axis. When numerator and denominator are equal (the default `1` / `1`), the ratio is 1 and no scaling is applied. Both keywords accept whole numbers in the range `1`-`2000`, so a wide range of rational ratios can be expressed (for example `3` / `2` for a 1.5:1 resolution difference). The controller forms the ratio (and its reciprocal) when either value changes, ready to apply during the vector move. Configure the pair on each member axis before starting the move, since it cannot be changed in motion.
