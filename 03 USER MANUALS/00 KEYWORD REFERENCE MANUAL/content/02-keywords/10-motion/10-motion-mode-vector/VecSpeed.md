...
Because the profile is generated for the resultant path and then split across the member axes by the geometry ([VecType](VecType.md)), no single member axis necessarily moves at `VecSpeed` — only the geometric resultant does. The controller re-reads `VecSpeed` every cycle, so raising or lowering it mid-move re-targets the path velocity on the next cycle. [VecPause](VecPause.md) temporarily forces the path-velocity target to 0; [StopVec](StopVec.md) does the same but ends the move.
