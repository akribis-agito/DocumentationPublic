...
`C` is the follower's position captured when [Begin](../04-motion-command/Begin.md) is issued; the cam shape is added on top of it, so the table value at the starting master position (which is positioned by [ECAMMasterIni](ECAMMasterIni.md)) is added as well. To avoid a step at start the cam table is normally authored so its starting entry is `0`. `cycle_offset` accumulates the height difference of the repeating segment across completed cycles (see [ECAMStartCyc](ECAMStartCyc.md) / [ECAMEndCyc](ECAMEndCyc.md)) so the follower advances continuously even when a cycle's first and last table values differ.
