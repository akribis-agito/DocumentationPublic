...
where `Ts` is the control sample time and `w = 2π·(PDPosFilt/100)`. Earlier firmware omitted the `2π` factor in the denominator (using `100 + Ts·PDPosFilt`), so the same `PDPosFilt` produced a slightly larger coefficient there; the minimum value (4150) yields `PDFiltFact = 1` under both forms. The minimum value (4150) is the smallest frequency that keeps the computed coefficient from rounding down to 0 (which would freeze the reference). A **higher** `PDPosFilt` means a faster filter (less smoothing, `PosRef` tracks the pulse stream more closely); a **lower** value means heavier smoothing.
