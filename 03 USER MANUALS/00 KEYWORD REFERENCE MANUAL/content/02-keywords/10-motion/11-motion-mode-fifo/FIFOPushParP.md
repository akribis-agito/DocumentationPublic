...
If the delta and cycle time resolve to an acceleration of magnitude below 16 384 counts/s² (one control-sample frequency at the standard 16 384 Hz rate — the smallest acceleration the per-sample velocity step can resolve), the segment faults the motion (motor off, fault 1031). Because the resolved acceleration is not known until the segment runs, this cannot be caught at push time the way [FIFOPushParA](FIFOPushParA.md) is.
