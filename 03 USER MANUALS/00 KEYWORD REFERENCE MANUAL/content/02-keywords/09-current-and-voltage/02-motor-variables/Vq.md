...
**Voltage feedforward (v5).** When [VoltageFFWOn](../../11-control-tuning/05-feedforwards/VoltageFFWOn.md) is non-zero, the q-axis voltage feedforward [VqFFW](../../11-control-tuning/05-feedforwards/VqFFW.md) is added to `Vq` immediately after the PI output is formed and before the vector saturation below. `VqFFW` carries the model-based resistive, inductive, back-EMF and d-q cross-coupling terms; under the default feedforward level settings it is zero, so `Vq` equals the PI output. In v4 there is no voltage feedforward and `Vq` is the PI output directly.

**Vector saturation.** Before the inverse Park transform, `Vq` and [Vd](Vd.md) are limited as a vector against the maximum PWM magnitude. If $\text{Vq}^2 + \text{Vd}^2$ exceeds the squared limit (the limit is multiplied by $\frac{4}{3}$ when the enhanced-speed-range bit of [ControlMode](ControlMode.md) is set), both `Vq` and `Vd` are scaled by the same factor so the sine-wave shape is preserved, and the voltage-saturation status bit in [StatReg](../../07-status-and-faults/StatReg.md) is set. In linear (amplitude) terms the $\frac{4}{3}$ on the squared limit is a factor of $\frac{2}{\sqrt{3}} \approx 1.1547$ on the magnitude: with enhanced speed range the `Vq`/`Vd` vector may reach $\text{MaxPWM} \times \frac{2}{\sqrt{3}}$, about 15.5 % more amplitude than the plain `MaxPWM` limit ($\text{Vq}^2 + \text{Vd}^2 \le \text{MaxPWM}^2$) used without it — the standard space-vector-over-sine gain that the common-mode injection on [Va](Va.md)/[Vb](Vb.md)/[Vc](Vc.md) makes usable.
