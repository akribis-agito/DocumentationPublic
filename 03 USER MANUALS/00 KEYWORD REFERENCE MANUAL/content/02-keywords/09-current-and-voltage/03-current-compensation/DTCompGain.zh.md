...
> **示例演算：** `DTCompGain = 0` 完全不叠加电压，驱动器的行为与未内置该补偿的驱动器相同。增益 1.0 施加该驱动器的标称死区校正量，更高的取值则按比例放大。
>
> 需要了解的关键结论是：误差曲线存在**极小值，而非平台区**。低于最优值时补偿不足，部分被窃取的伏秒仍未补回；高于最优值时驱动器过度补偿，误差会对称地重新增大。取值过高与过低同样错误，因此该值只能通过实测确定——而且最优值与死区时间**并非**线性关系，无法由某一驱动器的取值推算另一驱动器的取值。

> **注意：** 测量步骤及各驱动器的实测取值请参见《电流环补偿》应用笔记。
