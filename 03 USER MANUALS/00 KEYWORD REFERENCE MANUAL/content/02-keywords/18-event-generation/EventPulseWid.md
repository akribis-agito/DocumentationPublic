...
The output duration is produced by a hardware timer, so the width is quantized to the timer step. On central-i remote drives with incremental or SIN-COS feedback this is about **20 ns** for pulses up to roughly **163.8 us**, and **5.12 us** for longer pulses, up to a maximum of about **1.34 seconds**; stand-alone products use a different timer with a shorter maximum. Requested widths are quantized to the timer step (truncated toward the nearest lower step), and a width that exceeds the longest producible pulse is clamped to that maximum.
