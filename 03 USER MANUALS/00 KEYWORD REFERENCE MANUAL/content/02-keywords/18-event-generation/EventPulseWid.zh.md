...
使能事件时，控制器将宽度转换为脉冲生成器的内部时基，自动选择粗精或细精内部时间步长，使短脉冲和长脉冲均能精确计时。由于脉冲持续时间固定（而非固定位置长度），轴在脉冲期间移动的*距离*随速度增大而增大。下图展示了当轴跨越 [EventGap](EventGap.md) 所需时间短于脉冲宽度时，相邻脉冲如何重叠：
