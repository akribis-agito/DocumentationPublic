...
Like the base keyword, `ProgArgThisD` is only valid from within a running user program (it addresses the current call frame); issuing it from a plain communication command is rejected with a run-time error.
