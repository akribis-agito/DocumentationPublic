...
`Return` can also return values. It is issued as `AReturn[N], M`, where the **value** `M` is the number of *input* arguments the function received and the **array index** `N` encodes the number of *output* arguments as `N − 1` (so `AReturn[1]` returns no outputs, `AReturn[2]` one output, `AReturn[3]` two outputs, and so on). The output values themselves are not given here — the function writes them into its output-argument slots beforehand with [ProgArgThis](ProgArgThis.md); `Return` then pushes those slots onto the caller's numeric stack. The supported input-argument count is `0`–`10`, extended to `0`–`16` on central-i v5.
...
When `Return` runs it operates on the current thread's call stack. The command value is the count of *input* arguments the function received; the array index, minus one, is the count of *output* arguments to return (the output values having already been written into the function's output-argument slots with [ProgArgThis](ProgArgThis.md)). The engine then:
...
; function body
AReturn[1],0        ; return to the line after the call (no input or output arguments)

; end of a function that received 2 input arguments and returns 2 output values
; (the output values are written into the function's output-argument slots
;  with ProgArgThis before this line):
AReturn[3],2        ; array index 3 => 2 output arguments; value 2 => 2 input arguments
```
