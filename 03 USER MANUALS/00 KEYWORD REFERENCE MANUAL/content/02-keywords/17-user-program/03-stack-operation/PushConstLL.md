...
The only difference from [PushConstant](PushConstant.md) is the data type pushed: `PushConstLL` pushes a 64-bit signed integer literal rather than a 32-bit integer constant. The stack slot is the same; the typed forms simply control how the value is stored so it is interpreted correctly when consumed. A 64-bit value still occupies a single one of the thread's 50 stack slots, exactly like a 32-bit one, so it counts as one entry against the stack-full limit.
