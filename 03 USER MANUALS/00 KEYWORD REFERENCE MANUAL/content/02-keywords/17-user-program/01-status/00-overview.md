...
| …which thread the currently running program is executing on ("am I running, and on which thread?") | [ProgThread](./ProgThread.md) — central-i v5 only |

[ProgThread](./ProgThread.md) is the only keyword that lives in this folder; unlike the v4-available reads above, it is available on central-i v5 products only and reports the thread number that is currently executing.
