...
![各换相方法分别读取哪些 ComtMode 元素：`[1]`、`[5]` 和 `[19]` 适用于所有方法；搜索驱动类元素以及 v5 元素 `[27]`–`[32]` 仅由方法 0 读取，`[4]` 仅由方法 2 读取，`[20]`–`[24]` 仅由方法 5 读取，`[33]` 仅由方法 7 读取；方法 3、4 和 6 不读取任何配置元素](comtmode-element-map.svg)
