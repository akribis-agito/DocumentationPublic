...
| `[6]` | 平滑电压上升 | `0` 关闭，`1` 开启——斜坡方式施加搜索驱动而非阶跃（适用于重力负载/垂直轴）。对于方法 `5`，还会在每次跳动过程中连续推进磁通角，而非在两次跳动之间阶跃改变角度 |
...
| `[27]` | 定相**驱动模式** | `0` 电压模式（默认）——定相驱动以相电压 `Va`/`Vb`/`Vc` 形式施加；`1` 电流模式——定相驱动以电流指令形式施加，由电流环驱动各相。由方法 `0` 和 `5` 读取。既非 `0` 亦非 `1` 的值在写入时及参数恢复时被强制为 `0`。对于配置为外部驱动器的轴，电压模式会被拒绝（[ComtStatus](ComtStatus.md) `-17`）。仅限 Central-i v5 |
| `[28]` | 定相电流峰值 | 当 `[27]=1` 时，方法 `0` 或 `5` 每次跳动所爬升到的电流指令峰值，单位为 mA。电压模式下不读取该元素，此时方法 `0` 的峰值改为 `[2] × [3]`，方法 `5` 的峰值改为 `[20] × [21]`。不做限幅，且无非零默认值——在电流模式下若保持为 `0`，则不产生任何定相驱动。仅限 Central-i v5 |
...
多数元素仅由一种方法读取。下图为对应关系：

![各换相方法分别读取哪些 ComtMode 元素：`[1]`、`[5]` 和 `[19]` 适用于所有方法；搜索驱动类元素以及 v5 元素 `[29]`–`[32]` 仅由方法 0 读取，`[27]` 和 `[28]` 由方法 0 和 5 读取，`[4]` 仅由方法 2 读取，`[20]`–`[24]` 仅由方法 5 读取，`[33]` 仅由方法 7 读取；方法 3、4 和 6 不读取任何配置元素](comtmode-element-map.svg)
...
若启用 `[6]` 平滑，搜索驱动将以斜坡方式达到其最终值，而非阶跃施加，这可防止重力负载轴在换相开始时下落。方法 `0` 在索引 `[7]` 处的时间内达到该值；方法 `5` 则在跳动过程中完成爬升，并在跳动过程中连续推进磁通角，而非在两次跳动之间阶跃改变角度。
