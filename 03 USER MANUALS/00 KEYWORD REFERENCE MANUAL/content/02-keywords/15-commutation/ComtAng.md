...
For encoder-based commutation the angle comes from the feedback position within one electrical cycle (counts per electrical cycle = [EncRes](../03-encoder/01-general-settings/EncRes.md) / [PolePrs](../02-motor-and-amplifier/PolePrs.md), see [MotorType](../02-motor-and-amplifier/MotorType.md)); for Hall-based methods it comes from the [HallsValue](HallsValue.md) → [HallsAngle](HallsAngle.md) mapping (optionally smoothed by [HallOnlyFilt](HallOnlyFilt.md)). During burn-in motion ([BurnInMode](../../03-special-features/burn-in/BurnInMode.md) active, [ComtStatus](ComtStatus.md) `AComtStatus[1]` = `600`), the angle source is instead the open-loop angle driven by the controller at the burn-in frequency rather than feedback, so `ComtAng` sweeps continuously through 0.00°–359.99° as the test runs (the driven angle is range-clamped into 0°–360° each control cycle before advancing). The scaling shown above is exact: the firmware constant equals 360/2π × 100. The reported value is meaningful once a usable commutation angle is available, i.e. once the commutation-complete bit of [StatReg](../07-status-and-faults/StatReg.md) (bit 0) is set. For most methods that is when commutation finishes ([ComtStatus](ComtStatus.md) = `100`) or is not required (`200`); for the Hall-start switching methods (`ComtMode[1]=3` or `4`) the bit is already set at the rough stage ([ComtStatus](ComtStatus.md) `300`/`400`), so `ComtAng` is in use and meaningful from that point. `ComtAng` is reported for brushless motor types only; brush, voice-coil, simulation, and stepper types have no commutation angle.
