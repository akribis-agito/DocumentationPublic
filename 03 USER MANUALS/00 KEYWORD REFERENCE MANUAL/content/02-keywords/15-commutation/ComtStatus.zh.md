...
| -17 | 换相被拒绝：该方法会将定相电压直接施加到外部驱动器，这是不允许的。轴被关闭（仅限 Central-i v5）。 |

非法霍尔错误（`-7`、`-9`…`-12`）在 [HallsValue](HallsValue.md) 读取 `0` 或 `7`（合法范围 1–6 之外的两种组合）时，或观测到的霍尔序列与预期顺序不符时触发。

### 外部驱动器定相拒绝（状态 `-17`）

"跳零"（`ComtMode[1]=0`）和最小跳动搜索（`ComtMode[1]=5`）两种换相方法通过直接输出相电压（`Va`/`Vb`/`Vc`）来求解电角度——参见 [ComtMode](ComtMode.md)。这种电压不能送入外部驱动器，因此当轴配置为外部驱动器、且换相仍处于默认的电压模式（未切换为显式的电流模式）时，控制器拒绝定相：将 `ComtStatus[1]` 置为 `-17`，并关闭电机。

对于最小跳动搜索方法，这是最终状态。对于"跳零"方法，拒绝发生在该方法自身"跳动结束"检查的同一控制周期内，该检查随即发现电机已被关闭，并立即将状态覆写为 `-1`（意外电机关闭）——因此在该方法上 `-17` 是瞬态的，实际保留下来的是电机关闭且状态为 `-1`。无论哪种情况，结果相同：定相被拒绝，电机保持关闭。
