...
`CurrCmdHTime` 定义电流运行模式下电流参考的保持时间，单位为毫秒。其用法取决于 [CurrCmdSrc](CurrCmdSrc.md)：

- 若 `CurrCmdSrc` = 0 或 3（模拟量输入或从轴驱动器）：仅使用 `CurrCmdHTime[1]`，定义停留在电流运行模式内的时间。
...
|---|---|
| < 0 | 源值无限期保持。 |
| 0 | 轴退出电流运行模式并进入位置运行模式。 |
| > 0 | 源值保持 `CurrCmdHTime`，之后退出电流运行模式（`CurrCmdSrc` = 0 或 3）或进入下一对（`CurrCmdSrc` = 1 或 2）。对于 `CurrCmdSrc` = 1 或 2，若 [CurrCmdIndex](CurrCmdIndex.md) 达到最后一个可用条目（20）且该条目的 `CurrCmdHTime` 大于 0，则轴无限期保持最后一个 `CurrCmdVal` 值。 |
...
- **计数器最大值** — `2 000 000 000` 是用户可*写入* [CurrCmdCntr](CurrCmdCntr.md) 的最大值，而非运行时上限。在电流运行模式下，计数器只是每个控制周期累计一次，没有运行时饱和。
