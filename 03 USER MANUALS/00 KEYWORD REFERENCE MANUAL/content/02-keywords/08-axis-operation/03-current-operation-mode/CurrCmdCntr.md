...
1. If `CurrCmdSrc` = 0 or 3 (analog input, or the master axis whose current reference this axis follows): time elapsed under current operation mode.
...
`CurrCmdSrc` = 3 (follow another axis' current reference) is available only on **central-i (v5)**; on v4 the valid range of [CurrCmdSrc](CurrCmdSrc.md) is 0–2.
...
- **Sources 0 and 3 (analog / master axis):** if [CurrCmdHTime](CurrCmdHTime.md)`[1]` is non-negative the counter increments every control cycle and, when it exceeds `CurrCmdHTime[1]`, the axis switches back to position operation mode. If `CurrCmdHTime[1]` is negative the source is used indefinitely and the counter is not advanced. (Source 3 is central-i v5 only.)
