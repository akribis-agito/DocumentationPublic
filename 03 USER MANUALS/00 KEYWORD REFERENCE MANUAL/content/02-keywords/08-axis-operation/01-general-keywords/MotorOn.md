...
1. Unit hardware is healthy — the FPGA is not faulty, the dynamic-brake / variant / full-scale FPGA versions match the firmware, and (on central-i products) the firmware matches the board flash.
...
- **Inrush still active** — `MotorOn = 1` is rejected with an error (86) because the inrush relay has not closed yet; wait for the inrush charge resistor to be bypassed.
- **Drive too hot / overall current limit** (AGD301 32 A only) — `MotorOn = 1` is rejected with an error (241) because the summed continuous current limit (`ContCL[A]`+`[B]`+`[C]`) exceeds 28000 mA.
- **Gantry pair member disabled** — if either gantry member of a pair turns off, the other is automatically forced off with [ConFlt](../../07-status-and-faults/ConFlt.md) = `1061` (other gantry member got motor off) and gantry is cleared. See [GantryOn](../../12-gantry-control/01-general-variables/GantryOn.md).
...
- **Fault input asserted** — even after `MotorOn = 1` succeeds, a still-active fault input ([DInMode](../../05-inputs-outputs/04-digital-inputs/DInMode.md) code 24 / 26) will retrigger and disable the motor with [ConFlt](../../07-status-and-faults/ConFlt.md) = `1050` (code 24) or `1062` (code 26) and [MotorReason](../../07-status-and-faults/MotorReason.md) = `1`. `MotorReason = 2` is set only by the servo on/off digital input falling edge, not by a fault input.
...
- **Platform** — central-i products add a flash/firmware-mismatch check; standalone products skip it.
