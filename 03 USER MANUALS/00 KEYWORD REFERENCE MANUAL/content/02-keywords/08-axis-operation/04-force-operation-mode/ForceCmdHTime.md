...
| 0 | Axis exits force operation mode and enters position operation mode. For the table source (`ForceCmdSrc` = 1 or 2), when [ForcePIVOn](../../11-control-tuning/07-force-control/ForcePIVOn.md) is active the velocity integrator is seeded on the way out to avoid a current-reference jump. |
...
- **Zero value** — exits force mode at that entry; for the table source (`ForceCmdSrc` = 1 or 2) with `ForcePIVOn = 1` the velocity integrator is seeded on exit to avoid a current-ref jump.
