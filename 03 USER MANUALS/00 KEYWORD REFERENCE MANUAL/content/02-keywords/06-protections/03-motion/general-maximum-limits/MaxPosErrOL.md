...
- **Range overflow:** writes outside the keyword `range` are rejected with an out-of-range error; the stored value is left unchanged. The in-force internal limit is recomputed each time `MaxPosErrOL`/`MaxPosErr`/`OpenLoopOn`/`InjectType`/`InjectPoint` changes.
