...
- **Range overflow:** writes outside the keyword `range` are clamped. The active internal limit is recomputed each time `MaxVelErr`/`OpenLoopOn`/`InjectType`/`InjectPoint` changes; a write to `MaxVelErrOL` while already in open loop does not itself recompute the active limit, so it takes effect on the next such change (or the next entry into open loop / injection).
