...
- **Pseudo dual-loop ([DualEncSwapOn](../../../11-control-tuning/02-dual-loop-control/DualEncSwapOn.md) = 1):** the check runs only while true dual-loop is actually engaged — that is, while [DualLoopStat](../../../11-control-tuning/02-dual-loop-control/DualLoopStat.md) = 2. In pseudo dual-loop both loops use a single feedback source, so there is no two-feedback mismatch to test and the protection is suspended. With `DualEncSwapOn = 0` the protection is always active whenever `DualLoopOn` is non-zero. The same condition applies under range-limited switching ([DualEncMode](../../../11-control-tuning/02-dual-loop-control/DualEncMode.md) = 1): the check is active only inside the true dual-loop range.
