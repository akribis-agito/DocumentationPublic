...
Motor stuck protection works by comparing the current and velocity over a period of time. A stuck condition occurs when the velocity is less than a defined threshold while the current is greater than a defined threshold, over a defined period of time. If the stuck condition occurs, the axis is disabled and the error code is reported on [ConFlt](../../../07-status-and-faults/ConFlt.md) (ConFlt code 1007).

Criterion for stuck condition check:

- $abs(\text{Vel}\lbrack 3\rbrack)\ \le\ \text{StuckVel}$, **and**

- $abs(\text{MotorCurr})\ \ge\ \text{StuckCurr}$

for [StuckTime](StuckTime.md) consecutive control samples.
