...
The array is 1-indexed (index 0 is reserved); the highest usable index is one less than the frontmatter `array_size`. The blocks are laid out consecutively (the index ranges below show where values are *stored*):
