...
1. 命令静态制动器抱闸（断电），置位 [StatReg](../../07-status-and-faults/StatReg.md) 第 29 位的抱闸请求，并启动一个 `BrakeLockTime` 计时器，同时电机保持励磁；
2. 在计时器到时之前保持电机励磁——在此窗口内电机继续保持负载；
3. 当计时器到时，禁用电机。

第 29 位在命令抱闸的那一刻即置位，而不是等到计时器到时——因此在延时进行期间，所报告的状态已与制动器输出一致。等待窗口结束的是电机禁用动作。

请注意与 [BrakeRelTime](BrakeRelTime.md) 的不对称性：在释放侧，第 29 位在延时**结束**时（即允许运动时）被*清除*；而在抱闸侧，它在延时**开始**时即被*置位*。在两个方向上，该位都倾向于报告“已请求制动器保持”。
