...
### When several bits are set at once

The axis is disabled immediately if **any** enabled bit is set, but [ConFlt](../../07-status-and-faults/ConFlt.md) reports only a **single** code. Each protection condition is evaluated independently in a fixed order, and each match overwrites the `ConFlt` value, so when more than one condition is active simultaneously the last one in that order is the code you see. `ConFlt` therefore identifies *one* of the active conditions, not necessarily the first to assert or the most severe. To see the complete set of conditions that were active at the fault, read `HWProtectBits` itself (every active bit is present) or the diagnostic snapshot [ConFltSnapVal](../../07-status-and-faults/ConFltSnapVal.md)`[11]`, which holds the full bitfield captured at the moment of the fault.

On the AG100 (single-channel) drive, an STO2 condition (ConFlt code 1052) also drives the IPM-fault input, so STO2 is evaluated last to guarantee that a real STO2 event is reported as STO2 rather than as IPM fault (ConFlt code 1027). On that hardware, if both the STO2 and IPM-fault bits are set, `ConFlt` shows 1052.

### Edge cases

- **Motor off:** the bits continue to update from the live hardware signals every control cycle, but the step that turns a set bit into an axis-disabling [ConFlt](../../07-status-and-faults/ConFlt.md) runs only while the axis is enabled (motor on), not in simulation, and not on a position-drive amplifier. With the motor off, a condition can therefore be visible in `HWProtectBits` without having faulted the axis. When you next attempt motor-on, the still-present condition is re-checked at the enable request: if a latched hardware-protection bit is still set, the enable is refused and the matching `ConFlt` code is returned, so a latched STO/encoder/watchdog/IPM condition that is still asserted blocks the next motor-on until the underlying signal clears.
