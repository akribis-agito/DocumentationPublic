...
- **Motor off / non-current modes:** the over-current check runs only while the motor is on, the motor type is not the simulation type (see [MotorType](../../02-motor-and-amplifier/MotorType.md)), and the amplifier type is not a position-detector (PD) type (see [AmpType](../../02-motor-and-amplifier/AmpType.md)), where the current loop is bypassed. Whenever the check is skipped (motor off, simulation, or PD), the firmware resets the over-current counter, so the next time the check resumes it starts from a clean state.
