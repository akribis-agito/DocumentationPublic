...
- **Motor off:** the over-`MaxVBusAbs` status indication ([StatReg](../../07-status-and-faults/StatReg.md) bit 6) keeps updating, but the disabling trip itself fires only while the motor is on — the trip path is gated on the axis being enabled, the same as [MaxVBus](MaxVBus.md).
...
- **Range overflow:** writes outside `12000…95000` (mV) are rejected with an out-of-range error; the stored value is left unchanged. Set above [MaxVBus](MaxVBus.md) so the timed band acts first.
