...
The same supply-specific phase check is also evaluated when the axis is enabled: if the required phase(s) are missing at the moment of a [MotorOn](../../08-axis-operation/01-general-keywords/MotorOn.md) request, the enable is refused with fault code 1054.
...
- **Effect on [HWProtectBits](../01-general-protection/HWProtectBits.md):** the live `HWProtectBits` report still includes the AC-phase-missing bits regardless of the declared supply type; `PowerSupply` only governs whether those bits cause a trip. On Central-i the phase bits are excluded from the generic protection-trip path and are instead evaluated by the supply-specific phase check above.
...
- **Range overflow:** writes outside `1…3` are rejected with an out-of-range error; the stored value is left unchanged.
